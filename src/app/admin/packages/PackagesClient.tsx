...
import { getPackageById, type PackageRecord } from "./actions";

type PackagesClientProps = {
  packages: PackageRecord[];
  subscribersByPackage: Record<string, number>;
};
...
export function PackagesClient({ packages, subscribersByPackage }: PackagesClientProps) {
  ...
  const totalSubscriptions = Object.values(subscribersByPackage).reduce((sum, count) => sum + count, 0);
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            disabled={loadingEdit === pkg.id}
                            className="rounded-lg p-2 text-[var(--text-medium)] hover:bg-[var(--hover-bg)] hover:text-[var(--brand-primary)] transition-colors disabled:opacity-50"
                            title="Edit package"
                          >
