...
export async function POST(req: Request) {
  ...
  if (existing) {
    if (existing.driverId !== driverId) {
      return errorResponse("Subscription wash assigned to a different driver", 403);
    }

    // Update existing task
    await prisma.subscriptionDailyDriver.update({
    ...
    // Create new task
    await prisma.subscriptionDailyDriver.create({
      data: {
  ...
  return jsonResponse({ success: true, message: "Subscription wash started successfully" });
}
