...
export default async function DriverDashboardPage() {
  ...
  const activeBookings = bookings.filter((booking: DriverBookingItem) => {
    const paymentReceived =
      booking.payment?.status === "PAID" ||
      booking.status === "PAID" ||
      Boolean(booking.cashCollected && (booking.cashAmountCents ?? booking.service?.priceCents ?? 0) > 0);
    const isCompleted = booking.taskStatus === "COMPLETED";
    return !(isCompleted && paymentReceived);
  });
      ...
      <div className="grid gap-4">
        {activeBookings.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-[var(--surface-border)] bg-[var(--surface)]/70 p-6 text-center text-sm text-[var(--text-muted)]">
            All assigned jobs are cleared. Await new assignments.
          </div>
        ) : (
