...
async function debugSpotOrderIssues() {
  ...
  try {
    ...
    const spotOrders = await prisma.booking.findMany({
      where: {
        driverId: { not: null }, // Only bookings with drivers assigned
      },
      include: {
        Service: true,
    ...
    if (spotOrders.length === 0) {
      console.log('   ⚠️  No bookings with drivers found');
    } else {
      console.log(`   ✅ Found ${spotOrders.length} recent bookings with drivers:`);
